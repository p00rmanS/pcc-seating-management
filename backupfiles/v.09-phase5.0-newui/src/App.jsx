        ...
        if (remoteVenues) {
          ...
          seedRestaurants.forEach((restaurant) => {
            ...
            operational[restaurant.id] = {
              ...
              operations: remote?.operations || null,
            };
          });
...
const toggleAreaEditMode = useCallback(() => {
  ...
  const generateBulkTables = useCallback((config) => {
    ...
    const columns = Math.max(1, Math.min(30, Number(config.columns) || 1));
    const count = rows * columns;
    if (count > 300) return { ok: false, message: "Generate 300 tables or fewer at one time." };
    ...
    const tableType = config.tableType === "supervisor" ? "supervisor" : "regular";
    const horizontalGap = Math.max(0, Number(config.horizontalGap) || 0);
    const verticalGap = Math.max(0, Number(config.verticalGap) || 0);
    const offsetX = Math.max(0, Number(config.offsetX) || 0);
    const offsetY = Math.max(0, Number(config.offsetY) || 0);
    const stepX = 56 + horizontalGap;
    const stepY = 56 + verticalGap;

    const candidateNumbers = Array.from({ length: count }, (_, index) => String(startingNumber + index * increment));
    const existingNumbers = new Set(
      tables
        .filter((table) => {
          if (!config.replaceAreaTables) return true;
          return !(table.areaId === area.id || table.zone === area.label);
        })
        .map((table) => String(table.number))
    );
    const duplicateNumber = candidateNumbers.find((number) => existingNumbers.has(number));
    if (duplicateNumber) {
      return { ok: false, message: `Table ${duplicateNumber} already exists. Change the starting number or increment.` };
    }

    const nextTables = Array.from({ length: count }, (_, index) => {
      const row = Math.floor(index / columns);
      const column = index % columns;
      const rawX = area.x + offsetX + column * stepX;
      const rawY = area.y + offsetY + row * stepY;
      return {
        id: uid("t"),
        number: String(startingNumber + index * increment),
        capacity,
        zone: area.label,
        areaId: area.id,
        type: tableType === "supervisor" ? "super" : "regular",
        tableType,
        status: "available",
        statusUpdatedAt: null,
        serverId: null,
        ...
        childIds: null,
        pos: {
          x: Math.max(0, Math.min(layoutConfig.canvasWidth - 64, rawX)),
          y: Math.max(0, Math.min(layoutConfig.canvasHeight - 64, rawY)),
        },
      };
    });
    ...
    return {
      ...
      nextStartingNumber: startingNumber + count * increment,
    };
