...
export default function SeatingLayoutApp() {
  const [localSnapshot] = useState(() => loadLocalSnapshot());
...
const [selectedAreaId, setSelectedAreaId] = useState(null);
  ...
  const [currentRole, setCurrentRole] = useState(() => localSnapshot?.currentRole || "lead"); // "server" | "lead"
  const permissions = {
  ...
  useEffect(() => {
    ...
    const saveTimer = window.setTimeout(() => {
      try {
        const savedAt = saveLocalSnapshot({
          activeRid,
          currentRole,
          tablesByR,
          serversByR,
  ...
  useEffect(() => {
    ...
    const unsubscribeVenues = subscribeToCloudVenues(
      (remoteVenues) => {
        if (remoteVenues) {
          const operational = {};
          seedRestaurants.forEach((restaurant) => {
            ...
            operational[restaurant.id] = {
              tables: Array.isArray(remote?.tables) ? remote.tables : null,
              servers: Array.isArray(remote?.servers) ? remote.servers : null,
              groups: Array.isArray(remote?.groups) ? remote.groups : null,
              areas: Array.isArray(remote?.areas) ? remote.areas : null,
            };
          });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        await Promise.all(
          seedRestaurants.map((restaurant) =>
            saveVenueToCloud(restaurant.id, operational[restaurant.id], { clientId, role: currentRole })
          )
        );
        lastCloudSignatureRef.current = signature;
  ...
  }, [tablesByR, serversByR, groupsByR, areasByR, clientId, currentRole]);
  ...
  const updateTable = (id, patch) => {
    ...
    setTables((previous) =>
      previous.map((table) => {
        ...
        if (safePatch.status && safePatch.status !== table.status) {
          const changedAt = new Date().toISOString();
          logStatusChange({
            venueId: activeRid, entityType: "table", entityId: table.id,
            label: `Table ${table.number}`, fromStatus: table.status ?? "available",
            toStatus: safePatch.status, clientId, role: currentRole,
          }).catch((error) => console.error("Unable to write table activity log:", error));
          return { ...table, ...safePatch, statusUpdatedAt: changedAt };
        }
        return { ...table, ...safePatch };
      })
    );
  };
    ...
    setTables((previous) =>
      previous.map((table) => {
        ...
        logStatusChange({
          venueId: activeRid, entityType: "table", entityId: table.id,
          label: `Table ${table.number}`, fromStatus: table.status ?? "available",
          toStatus: nextStatus, clientId, role: currentRole,
        }).catch((error) => console.error("Unable to write table activity log:", error));
        return { ...table, status: nextStatus, statusUpdatedAt: changedAt };
      })
    );
  };
...
const updateArea = useCallback(
  ...
  [activeRid, clientId, currentRole, permissions.canManageZones, setAreas]
);
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3">
            <div className="flex flex-col items-end gap-1 text-xs text-slate-500">
              <span className="flex items-center gap-1.5" title="Automatic backup in this browser">
                <span className={`w-1.5 h-1.5 rounded-full ${saveDotClass}`} /> {saveLabel}
              </span>
              <span className="flex items-center gap-1.5" title="Firebase Realtime Database synchronization">
                <span className={`w-1.5 h-1.5 rounded-full ${
                  cloudState === "live" ? "bg-green-500" :
                  cloudState === "saving" || cloudState === "connecting" ? "bg-amber-500" : "bg-red-500"
                }`} />
                {cloudState === "live"
                  ? `Live sync${lastCloudSavedAt ? ` · ${new Date(lastCloudSavedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : " connected"}`
                  : cloudState === "saving" ? "Saving to cloud…"
                  : cloudState === "offline" ? "Offline · local backup active"
                  : cloudState === "error" ? "Cloud sync failed"
