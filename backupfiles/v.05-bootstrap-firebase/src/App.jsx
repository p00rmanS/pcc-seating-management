...
function SeatingWorkspace({ authSession }) {
  const [localSnapshot] = useState(() =>
    loadLocalSnapshot(authSession.user.uid, {
      allowLegacyMigration: authSession.profile.role === "lead" || authSession.profile.role === "admin",
    })
  );
...
const [selectedAreaId, setSelectedAreaId] = useState(null);
  ...
  const isLeadOrAdmin = currentRole === "lead" || currentRole === "admin";
  ...
  const updateTable = (id, patch) => {
    const allowedKeys = permissions.canEditLayout
      ? Object.keys(patch)
      : Object.keys(patch).filter((key) => key === "status" || key === "guestName");
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 bg-white border border-slate-300 rounded-xl px-3 py-2">
              ...
              <div className="leading-tight">
                <div className="text-xs font-semibold text-slate-700">{authSession.profile.displayName}</div>
                <div className="text-[10px] text-slate-500 capitalize">{currentRole} · {visibleRestaurants.map((venue) => venue.name).join(", ")}</div>
              </div>
              <button
                type="button"
                onClick={signOutEmployee}
                className="ml-2 text-xs px-2.5 py-1 rounded border border-slate-300 hover:bg-slate-50"
              >
                Sign out
              </button>
            </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {visibleRestaurants.map((r) => (
            <button
              key={r.id}
              onClick={() => {
                if (!authorizedVenueIds.includes(r.id)) return;
                setActiveRid(r.id);
                setSelectedTableId(null);
                setSelectedAreaId(null);
                setAreaEditMode(false);
