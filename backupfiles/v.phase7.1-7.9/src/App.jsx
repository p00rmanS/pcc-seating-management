...
const [blueprintsByR, setBlueprintsByR] = useState(() => Object.fromEntries(seedRestaurants.map((restaurant) => [restaurant.id, { dataUrl: null, opacity: 0.35, visible: true }])));
  ...
  const requestCanvasExpansion = useCallback((requiredWidth, requiredHeight) => {
    setCanvasSettingsByR((previous) => {
      ...
      const nextWidth = Math.min(50000, Math.max(current.width, Math.ceil(requiredWidth / 500) * 500));
      const nextHeight = Math.min(50000, Math.max(current.height, Math.ceil(requiredHeight / 500) * 500));
  ...
  const splitTable = (id, parts) => {
    ...
    setTables((prev) => {
      ...
      const children = parts.map((cap, i) => ({
        ...
        childIds: null,
        pos: {
          x: Math.max(0, parent.pos.x + i * spacing),
          y: Math.max(0, parent.pos.y + parentSize.height + 18),
        },
      }));
      firstChildId = children[0]?.id || null;
      const farthestChild = children[children.length - 1];
      if (farthestChild) requestCanvasExpansion(farthestChild.pos.x + parentSize.width + 600, farthestChild.pos.y + parentSize.height + 600);
